...
class GenRuleBuilder(base.BaseBuilder):
  ...
  def expand_cmd_labels(self):
    ...
    $(locations <foo>)    Space-delimited list of a dependency's output files.
    ...
    def _expand_onesrc():
      ...
      if len(outs) != 1:
        ...
        return os.path.join(self.buildroot, self.path_to_this_rule, outs[0])
    ...
    def _expand_makevar(re_match):
      ...
      tag_location = re.match(r'\s*location\s+([A-Za-z0-9/\-_:\.]+)\s*', tagstr)
      tag_locations = re.match(r'\s*locations\s+([A-Za-z0-9/\-_:\.]+)\s*',
                               tagstr)
      if tag_location:
    ...
    # Maybe try heuristic label expansion?  Actually on second thought that's a
    # terrible idea. Use the explicit syntax, you lazy slobs. ;-)

    # TODO: Maybe consider other expansions from the gnu make manual? $^ might
    # be useful.
...
class GenRule(base.BaseTarget):
  ...
  Arguments:
    ...
    deps: List of dependencies for this rule. Will eventually be deprecated in
          favor of srcs and tools. (List of labels; optional)
    executable: If true, declares the output of this rule to be executable.
                (Boolean; optional; default is False)
                Setting this to True means the output is an executable file and
                can be run using the "butcher run" command. The genrule must
                produce exactly one output in this case.
    cmd: The command to run. (String; required)
         ...
         path to the file denoted by <label>. If <label> is malformed, or not a
         declared dependency of this rule, or does not expand to exactly one
         file, the rule will fail. The label does not need to be a canonical
         address: "foo", ":foo", "//fulladdr/to/module:rulename" are all valid.
         The label may also be the name of an output file from this rule's
         "outs" attribute, in which case only the unqualified form (no // or :)
         may be used.
         2. All occurrences of $(locations <label>) are replaced by a
         space-separated list of paths to the files denoted by <label>. Similar
         to (1), the label must be a well-formed declared dependency of this
         rule, or one of its output files.
         3. $(OUTS) is replaced with a space-delimited list of full paths to
         this rule's output files. If the rule only has one output file, you
         may use $@ to refer to it.
  ...
  Notes:
    * $$ evaluates to $ (a literal dollar sign). For example, "$$$$.tmp" is a
      ...
      Similarly, to invoke a shell command containing dollar-signs such as "ls
      $(dirname $x), you should instead write "$((dirname $x)".
    * cmd should always use $(SRCS) or $(location <srclabel>) to refer to
      ...
      $(OUTS), $@, or $(location <outlabel>) to refer to output files. This is
      the only way to ensure that it reads and writes files in the right
    ...
    * In order to produce consistend and repeatible build results, you should
      ...
      specified as srcs or deps of this rule. To run a binary built by butcher,
      it is best to refer to it by label, as its filename or location on disk
      may change.

  Examples:

    # The following rule generates a tarball from its srcs using gnu tar, which
    # is built from a hypothetical //tools:gnutar rule elsewhere:
    genrule(
        name = "docs",
  ...
  # TODO: srcs should really work with both filenames in source tree and labels
  #       referring to other rules. It's going to take some considerable
  #       refactoring of how "addresses" are handled in butcher to enable that,
  #       but I'm pretty sure that is the right way to go.
